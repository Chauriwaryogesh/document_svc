...
public class DashboardService {
	...
	public DashboardStats getEmailCountStats(String userCode) {
		...
		long expiryDaysCount = security.stream().filter(sec -> sec.getEndTime() != null) // Filter out null endTime
				.filter(sec -> {
					try {
						LocalDate thresholdDate = LocalDate.now().plusDays(10);
						return sec.getEndTime().isAfter(thresholdDate);
					} catch (DateTimeParseException e) {
						System.err.println("Error parsing endTime: " + sec.getEndTime() + " for user "
								+ sec.getUserCode() + " - " + e.getMessage());
						return false;
					}
				}).count();
	...
	public DashboardStats getUsersCountStats(String userCode) {
		...
		try {
			...
			long userCodeVerified = security.stream().filter(sec -> sec.getIsUserCodeVerified().equalsIgnoreCase("Y"))
					.count();
			long adminAccess = customer.stream()
					.filter(sec -> sec.getAdminAccess() != null && sec.getAdminAccess().equalsIgnoreCase("Y")).count();
